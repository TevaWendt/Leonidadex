const fs=require('fs');
const V=JSON.parse(fs.readFileSync('releve/v-corrige.json','utf8'));
...
const SRCOFF=['Premier trailer','Second trailer','Captures officielles','Extended Look','Média officiel de Rockstar','Édition Ultimate',"Visuel officiel de l'édition Ultimate","Page officielle de l'édition Ultimate"];
...
V.forEach(v=>{
  ...
  if(v.st!=='comm'&&contreditVu(v.txt)&&!affirmeVu(v.txt)) add('contradiction',v.id+' : statut '+v.st+', texte nie la confirmation');
