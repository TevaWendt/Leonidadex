...
| `app.js`, `common.js`, `fiches.js`, `carte.js`, `comparateur.js`, `classement.js`, `progression.js`, `suivi.js` | le code qui fait fonctionner les pages (`suivi.js` : suivi générique par famille — un élément `data-track="famille" data-track-id="id"` reçoit un bouton « Je l’ai », une barre `data-track-bar` affiche le compte ; clés `lk_own_<famille>`) |
...
- Suivi personnel : familles `vehicules`, `armes` (fiches.js), `equipements`, `munitions` (suivi.js, attributs `data-track` dans armes.html), `lieux` (carte). `sync-site.cjs` lit les `data-track` d’armes.html pour `progression-data.js` (identifiants + noms). `progression-core.js` exporte et importe toutes ces clés ; ne jamais renommer une clé `lk_own_*` (le suivi des visiteurs en dépend).
- Piles d’images (`.lk-stack`) : chaque image est un lien, posé par `sync-site.cjs` via `outils/lot-c-visuals.cjs` (média → fiche du monde, élément d’acquisition, arme, véhicule, sinon `medias.html#media-<id>`).
