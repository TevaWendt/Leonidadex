...
- Tests automatisés (109 scénarios jsdom, dont 53 pour les collectibles : démarrage de chaque page, recherche, garage, équipement, comparateur, classement, carte, import/export) : `npm install jsdom postcss` une fois dans un dossier à part, puis depuis la racine `NODE_PATH=<ce dossier>/node_modules SITE_ROOT=$PWD node --test outils/tests/*.test.cjs`. Tout doit être `ok`.
