...
const pages=[...fs.readdirSync(root).filter(f=>f.endsWith('.html')),...['vehicules','armes'].flatMap(dir=>fs.readdirSync(path.join(root,dir)).filter(f=>f.endsWith('.html')).map(f=>dir+'/'+f))];
