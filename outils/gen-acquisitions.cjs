...
for(const c of source.categories.filter(c=>c.id!=='garages'&&!c.alias)){
 ...
 const body=`<section class="page-head shell"><p class="fiche-cat">GTA VI · contenus documentés</p><h1>${esc(c.label)}</h1><p class="lede">${esc(c.intro)}</p><p class="d-intro-note">${esc(c.limit)}</p>${contextual(c.id)}</section>${sectionsBlock(c)}
