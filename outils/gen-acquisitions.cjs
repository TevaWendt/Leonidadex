...
function sectionsBlock(c){if(!Array.isArray(c.sections)||!c.sections.length)return '';return `<section class="shell d-section d-topics" aria-labelledby="topics-title"><h2 id="topics-title">${c.id==='style'?'Les quatre volets du style':'Ce qu’il faut savoir'}</h2><div class="d-grid d-topic-grid">${c.sections.map(x=>`<article class="d-card d-topic" id="${esc(x.id)}" data-d-reveal><div class="d-card-body"><p class="d-label">${esc(x.status||'')}</p><h3>${esc(x.title)}</h3><p>${esc(x.text)}</p>${x.link?`<div class="d-actions"><a href="${esc(x.link)}">${esc(x.linkLabel||'Voir')}</a></div>`:''}</div></article>`).join('')}</div></section>`;}
...
for(const c of source.categories.filter(c=>c.id!=='garages'&&!c.alias)){
 ...
 const body=`<section class="page-head shell lk-glow">${pile?'<div class="lk-head-grid"><div>':''}<p class="fiche-cat">GTA VI · contenus documentés</p><h1>${esc(c.label)}</h1><p class="lede">${esc(c.intro)}</p><p class="d-intro-note">${esc(c.limit)}</p>${contextual(c.id)}${pile?'</div>'+pile+'</div>':''}</section>${sectionsBlock(c)}
...
fs.writeFileSync(path.join(__dirname,'acquisitions-index.json'),JSON.stringify([...source.categories.filter(c=>!c.alias).map(c=>({l:c.label,k:'Contenus documentés',u:c.route,s:c.label.toLowerCase()})),...source.categories.filter(c=>!c.alias&&Array.isArray(c.sections)).flatMap(c=>c.sections.map(x=>({l:x.title,k:c.label,u:c.route+'#'+x.id,s:(x.title+' '+x.text).toLowerCase()}))),...items.map(x=>({l:x.name,k:cat(x.category).label,u:x.hubUrl,s:(x.name+' '+x.description).toLowerCase()}))]));
