...
async function persistenceAndShare(page, context, browser, origin) {
  ...
  check(await page.locator('#saved-list [data-load]').count() > 0, 'Saved configuration appears in the notebook');
  await page.reload({ waitUntil: 'networkidle' });
  await page.locator('.calc-saved summary').click();
...
async function domMain() {
  ...
  try {
    ...
    check(page.d.querySelectorAll('#saved-list [data-load]').length === 1, 'Favourite creates a notebook entry');
    fill(page, '#f-goal-capital', '222222');
    page.d.querySelector('#saved-list [data-load]').click();
    ...
    check(favouritesReloaded.d.querySelectorAll('#saved-list [data-load]').length === 1, 'Favourites persist across fresh documents');
