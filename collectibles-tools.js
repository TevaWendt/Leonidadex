...
(function () {
  ...
  function saveView(name) {
    ...
    if (typeof name !== 'string' || !name.trim() || name.trim().length > 64) throw new Error('Donne un nom de 1 à 64 caractères à cette recherche.');
    const query = canonicalQuery(), next = core.getToolsState();
    if (next.savedViews.length >= 12) throw new Error('Tu as déjà 12 recherches. Retires-en une avant d’enregistrer une nouvelle vue.');
  ...
  function addToPlan(id) {
    ...
    if (next.plan.length >= 30) throw new Error('Ta sortie contient déjà 30 étapes. Retire une étape pour en ajouter une.');
  ...
  function renderSavedViews() {
    ...
    $('col-saved-views').innerHTML = toolState.savedViews.length ? '<ul class="col-tools-list">' + toolState.savedViews.map(entry => '<li><button type="button" class="col-saved-recall" data-col-view-recall="' + esc(entry.id) + '"' + (!hasCatalogue ? ' disabled' : '') + '><strong>' + esc(entry.name) + '</strong><span>Rouvrir les filtres enregistrés ↗</span></button><button type="button" class="col-tools-icon-btn" data-col-view-delete="' + esc(entry.id) + '" aria-label="Supprimer la recherche ' + esc(entry.name) + '">×</button></li>').join('') + '</ul>' : '<p class="col-tools-empty">' + (hasCatalogue ? 'Règle les filtres du catalogue, puis donne un nom à cette vue pour la retrouver en un clic.' : 'Dès que des fiches existeront, filtre le catalogue, nomme la vue et retrouve-la ici.') + '</p>';
  ...
  function renderPlan() {
    ...
    }).join('') : '<li class="col-tools-empty">' + (hasCatalogue ? 'Ajoute des fiches avec « Ajouter à ma sortie », puis choisis leur ordre ici.' : 'Ta prochaine sortie prendra forme avec les premières fiches documentées. Aucun emplacement n’est inventé.') + '</li>';
  ...
  async function shareView() {
    ...
    const note = localFilter ? ' Le filtre de collection utilisera le carnet local de la personne qui ouvre ce lien ; tes trouvailles et notes ne sont pas partagées.' : ' Le lien contient uniquement les filtres, aucune note ni sauvegarde personnelle.';
    try {
      ...
      if ($('col-share-fallback')) {
        ...
        message('La copie automatique est indisponible. Copie le lien affiché dans le champ.' + note);
      } else message('La copie automatique est indisponible. Copie l’adresse de la page dans ton navigateur.');
    }
  }
  if (host && view) {
    host.innerHTML = '<div class="col-tools-heading"><div><p class="col-eyebrow">À TA FAÇON</p><h2>Préparer la prochaine sortie.</h2></div><p>Tes recherches, ton ordre d’exploration, ta checklist.</p></div><div class="col-tools-grid"><section class="col-tools-panel"><div class="col-tools-panel-head"><h3>Mes recherches</h3><p class="col-tools-what">Une recherche, c’est un jeu de filtres (région, famille, statut, mot-clé) enregistré sous un nom. Tu la rappelles d’un clic au lieu de tout recocher.</p><span id="col-saved-count">0 / 12</span></div><div id="col-saved-views"></div><form id="col-save-view-form"><label for="col-view-name">Nom de cette vue</label><div class="col-save-view-row"><input id="col-view-name" maxlength="64" placeholder="Ex. Mes favoris à Vice City" autocomplete="off" required><button type="submit" id="col-save-view" class="col-btn">Enregistrer</button></div></form><p class="col-tools-fine">Les filtres sont conservés, pas une copie figée des résultats. Tes vues suivent les prochaines publications.</p></section><section class="col-tools-panel col-plan-panel"><div class="col-tools-panel-head"><h3>Ma sortie</h3><p class="col-tools-what">Ta feuille de route pour la prochaine session : les objets à aller chercher, dans l’ordre que tu choisis, jusqu’à 30 étapes.</p><span id="col-plan-count">0 / 30</span></div><p class="col-tools-intro">Choisis tes étapes et leur ordre. Cette liste personnelle ne calcule ni trajet optimal ni temps de déplacement.</p><ol id="col-plan-list" class="col-plan-list"></ol><div id="col-plan-next"></div></section><section class="col-tools-panel"><div class="col-tools-panel-head"><h3>Emporter la checklist</h3><p class="col-tools-what">La liste affichée, exportée en fichier CSV ou mise en page pour l’imprimer, à cocher sur un second écran ou sur papier pendant que tu joues.</p><span aria-hidden="true">↗</span></div><p id="col-tools-result-count" class="col-tools-intro"></p><div class="col-tools-export-options"><label class="col-checkbox"><input type="checkbox" id="col-export-notes"> Inclure mes notes personnelles</label><label class="col-checkbox"><input type="checkbox" id="col-export-spoilers"> Inclure les lieux précis et récompenses</label></div><div class="col-tools-export-actions"><button type="button" id="col-export-csv" class="col-btn">Exporter la vue en CSV ↓</button><button type="button" id="col-print-checklist" class="col-btn">Préparer l’impression</button><button type="button" id="col-share-view" class="col-text-button">Partager cette vue ↗</button></div><div id="col-share-fallback" hidden><label for="col-share-url">Lien à copier manuellement</label><input id="col-share-url" type="text" readonly></div><p class="col-tools-fine">Le CSV et l’impression suivent tes filtres. La sauvegarde JSON du carnet inclut aussi tes recherches et ta sortie.</p></section></div><p id="col-tools-storage" class="col-notice col-warning" hidden></p><p data-col-tools-feedback class="col-tools-feedback" role="status" aria-live="polite"></p><dialog id="col-print-dialog" class="col-print-dialog" aria-labelledby="col-print-title"><div class="col-print-toolbar"><button type="button" id="col-do-print" class="col-btn col-btn-primary">Imprimer / Enregistrer en PDF</button><button type="button" id="col-close-print" class="col-btn">Fermer l’aperçu</button></div><div id="col-print-content"></div></dialog>';
    ...
    $('col-export-csv').addEventListener('click', () => { try { downloadCSV(); } catch (_) { message('Le téléchargement CSV n’a pas pu être préparé. Ton carnet est conservé.'); } });
    ...
    $('col-do-print').addEventListener('click', () => {
      ...
      try { window.print(); } catch (_) { document.body.classList.remove('col-printing'); message('L’impression est indisponible dans ce navigateur. Tu peux exporter la vue en CSV.'); }
    });
  ...
  document.addEventListener('click', event => {
    ...
    try {
      if (button.hasAttribute('data-col-plan')) {
        ...
        if (core.getToolsState().plan.includes(id)) { removeFromPlan(id); message('Étape retirée de ta sortie.'); }
        else { addToPlan(id); message('Fiche ajoutée à ta sortie. Modifie son ordre depuis le catalogue.'); }
        ...
        removeFromPlan(button.dataset.colPlanRemove); message('Étape retirée de ta sortie.'); ($('col-plan-list')?.querySelector('button:not(:disabled)') || $('col-plan-list'))?.focus({preventScroll: true});
