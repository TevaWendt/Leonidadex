...
(function () {
  ...
  function renderSavedViews() {
    ...
    $('col-saved-views').innerHTML = toolState.savedViews.length ? '<ul class="col-tools-list">' + toolState.savedViews.map(entry => '<li><button type="button" class="col-saved-recall" data-col-view-recall="' + esc(entry.id) + '"' + (!hasCatalogue ? ' disabled' : '') + '><strong>' + esc(entry.name) + '</strong><span>Rouvrir les filtres enregistrés ↗</span></button><button type="button" class="col-tools-icon-btn" data-col-view-delete="' + esc(entry.id) + '" aria-label="Supprimer la recherche ' + esc(entry.name) + '">×</button></li>').join('') + '</ul>' : '<p class="col-tools-empty">' + (hasCatalogue ? 'Réglez les filtres du catalogue, puis donnez un nom à cette vue pour la retrouver en un clic.' : 'Dès que des fiches existeront, filtrez le catalogue, nommez la vue et retrouvez-la ici.') + '</p>';
