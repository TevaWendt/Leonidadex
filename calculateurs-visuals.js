...
function create(h){
 ...
 function chart(id,o){
  ...
  const yt=niceTicks(yLo,yHi,4),xt=niceTicks(xLo,xHi,5);
  const grid=yt.map(v=>'<line class="gridline" x1="'+L+'" x2="'+(W-R)+'" y1="'+py(v).toFixed(1)+'" y2="'+py(v).toFixed(1)+'"/><text class="c-tick" x="'+(L-8)+'" y="'+(py(v)+4).toFixed(1)+'" text-anchor="end">'+esc(o.unit==='$'?compact.format(v)+' $':o.unit==='%'?nf.format(v)+' %':nf.format(v))+'</text>').join('')+xt.map(v=>'<line class="gridline c-grid-x" y1="'+T+'" y2="'+(H-B)+'" x1="'+px(v).toFixed(1)+'" x2="'+px(v).toFixed(1)+'"/><text class="c-tick" x="'+px(v).toFixed(1)+'" y="'+(H-B+18)+'" text-anchor="middle">'+esc(o.xUnit==='h'?hours(v):nf.format(v))+'</text>').join('');
  ...
  const paths=series.map((s,i)=>{const pt=PATTERN[s.kind]||PATTERN.forecast;const d=s.points.map((p,k)=>(k?'L':'M')+px(p.x).toFixed(1)+' '+py(p.y).toFixed(1)).join(' ');const marks=s.points.map(p=>'<g class="c-chart-point c-series-'+i+'" tabindex="0" role="img" aria-label="'+esc(s.name+' · '+(p.label||fmt(p.x,o.xUnit))+' : '+fmt(p.y,o.unit))+'"><title>'+esc(s.name+' · '+(p.label||fmt(p.x,o.xUnit))+' : '+fmt(p.y,o.unit))+'</title>'+marker(pt.marker,px(p.x).toFixed(1),py(p.y).toFixed(1),p.key?6:4)+'</g>').join('');return (s.points.length>1?'<path class="path c-series-'+i+'" d="'+d+'"'+(pt.dash?' stroke-dasharray="'+pt.dash+'"':'')+'/>':'')+marks;}).join('');
  const legend=series.length>1||o.legendAlways?'<ul class="c-legend">'+series.map((s,i)=>{const pt=PATTERN[s.kind]||PATTERN.forecast;return '<li><svg viewBox="0 0 40 16" width="40" height="16" aria-hidden="true"><line class="path c-series-'+i+'" x1="2" x2="38" y1="8" y2="8"'+(pt.dash?' stroke-dasharray="'+pt.dash+'"':'')+'/><g class="c-chart-point c-series-'+i+'">'+marker(pt.marker,20,8,4)+'</g></svg>'+esc(s.name)+(s.kind==='realized'?' (réalisé)':s.kind==='hypothesis'?' (hypothèse)':s.kind==='reference'?' (repère)':'')+'</li>';}).join('')+'</ul>':'';
  const rows=[];const xsAll=[...new Set(all.map(p=>p.x))].sort((a,b)=>a-b);xsAll.forEach(x=>{rows.push('<tr><th scope="row">'+esc(o.xUnit==='h'?hours(x):nf.format(x)+(o.xUnit?' '+o.xUnit:''))+'</th>'+series.map(s=>{const p=s.points.filter(q=>q.x===x);return '<td>'+(p.length?p.map(q=>esc(fmt(q.y,o.unit))).join(' → '):'—')+'</td>';}).join('')+'</tr>');});
  return '<figure class="calc-chart c-chart" data-c-chart="'+esc(id)+'"><figcaption class="c-chart-head"><strong id="'+esc(id)+'-title">'+esc(o.title)+'</strong>'+(o.question?'<span class="c-chart-question">'+esc(o.question)+'</span>':'')+'</figcaption><svg viewBox="0 0 '+W+' '+H+'" role="img" aria-labelledby="'+esc(id)+'-title '+esc(id)+'-reading"><text class="c-axis-title" x="'+L+'" y="'+(T-16)+'">'+esc(o.yTitle||'')+'</text><text class="c-axis-title" x="'+(W-R)+'" y="'+(H-B+40)+'" text-anchor="end">'+esc(o.xTitle||'')+'</text>'+grid+zero+thresholds+paths+'</svg>'+legend+'<p class="c-chart-reading" id="'+esc(id)+'-reading"><strong>À retenir :</strong> '+esc(o.reading||'')+'</p>'+(o.note?'<p class="calc-note">'+esc(o.note)+'</p>':'')+'<details class="c-chart-data"><summary>Les chiffres du graphique</summary><div class="calc-table-wrap" tabindex="0"><table class="calc-table"><caption>'+esc(o.title)+'</caption><thead><tr><th scope="col">'+esc(o.xTitle||'x')+'</th>'+series.map(s=>'<th scope="col">'+esc(s.name)+'</th>').join('')+'</tr></thead><tbody>'+rows.join('')+'</tbody></table></div></details></figure>';
 }
 ...
 function sensitivity(tool,sens,metric){
  ...
  return chart('c-sensitivity-'+tool,{title:'Et si le chiffre bouge de 20 % ?',question:label,xTitle:sens.label,yTitle:label+(unit?' ('+unit+')':''),unit:unit,series:[{name:'Résultat recalculé',kind:'hypothesis',points:sens.rows.map((x,i)=>({x:i,y:x.result[key],label:x.factor===1?'Ton chiffre':x.factor<1?'−20 %':'+20 %',key:x.factor===1}))}],reading:'Entre −20 % et +20 %, le résultat va de '+fmt(Math.min(...vals),unit)+' à '+fmt(Math.max(...vals),unit)+'. Ce sont trois recalculs, pas une prévision.'});
 }
 function roi(s,r,d){
  if(!r.valid)return '';
